...
from data_merge import data_merger

def data_preprocesser():
    # Run data_merge.py if file doesn't exist
    ...
    # Rounding departure delay to one of [0, 15, 30, 60, 120, 240]
    round_values = np.array([0, 15, 30, 60, 120, 240])

    # Convert the departure delay column values to float
    vals = df["Departure Delay"].values.astype(float)

    # For each value, find the nearest value in round_values
    df["Departure Delay"] = round_values[np.argmin(np.abs(vals[:, None] - round_values), axis=1)]

    # Standardising integers values to range [0, 1]
    int_col = ["Weather Delay", "Departure Delay", "Carrier Delay", "National Air System Delay", "Security Delay", 
    ...
    # Saving scalers
    with open("model_files/scalers.pkl", "wb") as file:
